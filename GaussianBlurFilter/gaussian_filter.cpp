...
int main(int argc, char *argv[])
{
	...
	char* outimage; // the name of the output image file, taken by the arguments
	char mychar;

	//read in the program's arguments
	if (readArguments(argc, argv, &imgname, &gaussianSize, &gaussianSigma, &outimage) == false)
		return -1;

	printf("gaussianSize, %d\n", gaussianSize);
	...
	//gaussian kernel 5x5
	/*float lFilter[WINDOW_SIZE*WINDOW_SIZE] = { 0.002969 / 0.997026, 0.013306 / 0.997026, 0.021938 / 0.997026, 0.013306 / 0.997026, 0.002969 / 0.997026,
		0.013306 / 0.997026, 0.059634 / 0.997026, 0.098320 / 0.997026, 0.059634 / 0.997026, 0.013306 / 0.997026,
		0.021938 / 0.997026, 0.098320 / 0.997026, 0.162103 / 0.997026, 0.098320 / 0.997026, 0.021938 / 0.997026,
		0.013306 / 0.997026, 0.059634 / 0.997026, 0.098320 / 0.997026, 0.05963 / 0.997026, 0.013306 / 0.997026,
		0.002969 / 0.997026, 0.013306 / 0.997026, 0.021938 / 0.997026, 0.013306 / 0.997026, 0.002969 / 0.997026
	};
	memcpy(filter, lFilter, WINDOW_SIZE*WINDOW_SIZE*sizeof(float));*/

	filter[gaussianSize * gaussianSize];
	filter = createGaussianKernel(gaussianSize, gaussianSigma);
    ...
    int width, height;
    imageObjects[0] = LoadImage(context, argv[2], width, height);
    if (imageObjects[0] == 0)
    {
	...
	cl_mem ocl_filter = clCreateBuffer(
		context,
		CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
		gaussianSize*gaussianSize*sizeof(float),
		filter,
		&errNum);
	
	if (errNum != CL_SUCCESS)
	{
		std::cerr << "Error creating ocl_filter buffer." << std::endl;
		Cleanup(context, commandQueue, program, kernel, imageObjects, sampler);
		return 1;
	}
    ...
    program = CreateProgram(context, device, "gaussian_filter.cl");
    if (program == NULL)
    {
        Cleanup(context, commandQueue, program, kernel, imageObjects, sampler);
		system("pause");
    ...
    kernel = clCreateKernel(program, "gaussian_filter_kernel", NULL);
    if (kernel == NULL)
    {
	...
	cl_mem ocl_filter_x_gradient = clCreateBuffer(
		context,
		CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
		gaussianSize*gaussianSize*sizeof(float),
		filter,
	...
	cl_mem ocl_filter_y_gradient = clCreateBuffer(
		context,
		CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
		gaussianSize*gaussianSize*sizeof(float),
		filter,
	...
	int windowSize = gaussianSize;
	errNum = clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*)&imageObjects[0]);
	errNum = clSetKernelArg(kernel, 1, sizeof(cl_mem), (void*)&imageObjects[1]);
	errNum = clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*)&ocl_filter);
	errNum = clSetKernelArg(kernel, 3, sizeof(int), (void*)&windowSize);
    ...
    if (!SaveImage(argv[8], buffer, width, height))
    {
        std::cerr << "Error writing output image: " << argv[8] << std::endl;
        Cleanup(context, commandQueue, program, kernel, imageObjects, sampler);
        delete [] buffer;
        return 1;
    }
