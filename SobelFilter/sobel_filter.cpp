...
int main(int argc, char** argv)
{
    ...
    program = CreateProgram(context, device, "sobel_filter.cl");
    if (program == NULL)
    {
        Cleanup(context, commandQueue, program, kernel, imageObjects, sampler);
		system("pause");
    ...
    kernel = clCreateKernel(program, "sobel_filter_kernel", NULL);
    if (kernel == NULL)
    {
	...
	errNum = clSetKernelArg(kernel, 2, sizeof(cl_mem), (void*)&ocl_filter_x_gradient);
	errNum = clSetKernelArg(kernel, 3, sizeof(cl_mem), (void*)&ocl_filter_y_gradient);
	errNum = clSetKernelArg(kernel, 4, sizeof(int), (void*)&windowSize);
