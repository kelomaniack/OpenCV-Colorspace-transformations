...
int main(int argc, char** argv)
{
    ...
    program = CreateProgram(context, device, "median_filter.cl");
    if (program == NULL)
    {
        Cleanup(context, commandQueue, program, kernel, imageObjects, sampler);
		system("pause");
    ...
    kernel = clCreateKernel(program, "median_filter_kernel", NULL);
    if (kernel == NULL)
    {
	...
	errNum = clSetKernelArg(kernel, 2, sizeof(int), (void*)&windowSize);
